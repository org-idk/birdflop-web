...
import { sortColors, invertColors, reverseColors, cloneColors } from '~/util/rgb/RGBUtils';
import { ColorPicker } from '@luminescent/ui-qwik';
import { Plus, Copy, RefreshCw, ArrowUpDown } from 'lucide-icons-qwik';
...
export default component$(({ id = 'text' }: { id?: string }) => {
  ...
  return (
    <div
      class={{
        'flex flex-col mb-s5': true,
        hidden: rgbStore.disperse,
      }}
    >
      <div
        class="w-full h-2 rounded-full items-center relative"
        id={'colormap' + id}
        style={`background: ${
        ...
        onMouseDown$={(e, el) => {
          ...
          if (id === 'shadow') rgbStore.syncshadow = false;
      ...
      >
        ...
        {colors.value.map((color, i) => (
          <div
            ...
            class={{
              'absolute -mt-1.5 -ml-3 w-5 h-5 hover:scale-125 rounded-full lum-bg drop-shadow-md transition-transform':
                true,
            }}
            style={{
              '--bg-color': color.hex,
        ...
        <div
          ...
          class={{
            ...
            'flex-col gap-2 motion-safe:transition-all absolute top-full z-1000 mt-2':
              true,
